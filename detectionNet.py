...
with tf.Session(graph = graph) as sess:
    ...
    while db.batches_elapsed < 1000:
        ...
        print('X loss: %.2f | Y Loss: %.2f | W loss: %.2f | H Loss: %.2f | Cobj loss: %.2f | CNobj Loss: %.2f | P loss: %.2f' % (np.mean(out[3]),np.mean(out[4]),np.mean(out[5]),np.mean(out[6]),np.mean(out[7]),np.mean(out[8]),np.mean(out[9])))
